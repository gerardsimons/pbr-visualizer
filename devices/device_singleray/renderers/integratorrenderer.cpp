...
namespace embree
{
	...
	IntegratorRenderer::RenderJob::RenderJob (Ref<IntegratorRenderer> renderer, const Ref<Camera>& camera, const Ref<BackendScene>& scene,
											  const Ref<ToneMapper>& toneMapper, Ref<SwapChain > swapchain, int accumulate, int iteration, DataConnector* dataConnector)
	: renderer(renderer), camera(camera), scene(scene), toneMapper(toneMapper), swapchain(swapchain),
	accumulate(accumulate), iteration(iteration), tileID(0), atomicNumRays(0), dataConnector(dataConnector)
	{
		numTilesX = ((int)swapchain->getWidth() +TILE_SIZE-1)/TILE_SIZE;
		numTilesY = ((int)swapchain->getHeight()+TILE_SIZE-1)/TILE_SIZE;
		rcpWidth  = rcp(float(swapchain->getWidth()));
...
//                    }
    ...
    void IntegratorRenderer::RenderJob::renderTileWithDataConnector(size_t threadIndex, size_t threadCount, size_t taskIndex, size_t taskCount, TaskScheduler::Event* event)
    {
        printf("Render tile with data connector......\n\n");
